...
event_filepath = fullfile(main_dir, 'fMRI_conditions_for_fast_vs_slow.mat');
...
for subject_number = 1 : total_subjects
    ...
    spm_dir = fullfile(path, 'SPM model for fast vs slow conditions');
...
second_stat_dir = fullfile(main_dir, 'Second Level Stats for fast vs slow conditions');
...
for subject_number = 1 : total_subjects
    clearvars path name ext
    [path,name,ext] = fileparts(niifiles{subject_number,1});
    spm_dir(subject_number,:) = fullfile(path, 'SPM model for fast vs slow conditions', 'SPM.mat');
end
...
condition_contrast =    [1      -1
    ...
    main_dir = params.main_dir; % Specify your main directory
    spm_dir = fullfile(main_dir, 'Second Level Stats for fast vs slow conditions');
