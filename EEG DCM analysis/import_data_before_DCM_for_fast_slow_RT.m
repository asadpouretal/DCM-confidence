...
result_path = params.fast_slow_result_path;
...
for EEG_File_Number = 1 : length(FileInfo)
    ...
    clearvars -except FileInfo EEG_File_Number Event_Types Event_Path result_path main_dir SPM_Path SPM_Ready_File SPM_output defaultProfileName params
    eeglab
    close
    close all
...
%     [filename, pathname] = uigetfile({'*.mat'},'Select a .mat file to load your fMRI Acquisition data from','F:\Raw Data\fMRI - EEG\Sabina''s Decision Making\sub-01\EEG');
    ...
    Cluster_Centroids = params.cluster_centroids;   % reaction time thresholds for fast and slow RTs (ms)
...
%     accuracy(accuracy == 0) = [];
    ...
    subject_number = ceil(EEG_File_Number / 2);
    Conficence_Value(RT < Cluster_Centroids{subject_number}(1)) = 'F';
    Conficence_Value(RT >= Cluster_Centroids{subject_number}(1) & RT <= Cluster_Centroids{subject_number}(2)) = 'M';
    Conficence_Value(RT > Cluster_Centroids{subject_number}(2)) = 'S';
    ...
    %--------------------------------------------------------------------------
    save(fullfile(main_dir, 'eeglab_analysed_data_files_fast_slow.mat'),"SPM_output","SPM_Ready_File","SPM_Path")
...
for subject_number = 1 : length(SPM_output)
    ...
    subject_run(subject_number).S.trialdef(1).conditionlabel = params.fast_slow_condition_name{1};
    subject_run(subject_number).S.trialdef(1).eventtype = Event_Types(1);
    subject_run(subject_number).S.trialdef(1).eventvalue = 'F';
    subject_run(subject_number).S.trialdef(1).trlshift = 0;

    subject_run(subject_number).S.trialdef(2).conditionlabel = params.fast_slow_condition_name{2};
    subject_run(subject_number).S.trialdef(2).eventtype = Event_Types(1);
    subject_run(subject_number).S.trialdef(2).eventvalue = 'S';
...
parfor subject_number = 1 : length(SPM_output)
...
parfor subject_number = 1 : subject_number - 1
    cd(subject_run(subject_number).folder)
    S = subject_run(subject_number).S;
    D = spm_eeg_merge(S);
end
