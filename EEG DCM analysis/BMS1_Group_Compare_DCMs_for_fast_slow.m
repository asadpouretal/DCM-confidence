...
within_model_names = {{''}; 
    {''};
    {''}};
Model_numbers = params.DCM_model_num;
...
for condition_num = 1 : length(condition_names)
    ...
    main_dir = fullfile(params.fast_slow_result_path, 'SPM Analyses', params.DCM_folder_name);
    ...
    for model_number = 1 : Model_numbers(condition_num)
        for within_model = 1 : length(within_model_names{condition_num})
            model_file_names{within_model, model_number} = ['DCM_CMC_M' num2str(model_number) within_model_names{condition_num}{within_model} '_Fast_Slow.mat'];
        end
    end
    ...
    [GCM,subject_nums, inference_dir] = GCM_extract(main_dir,inference_method, inference_folder_name, model_file_names, dir_name_for_inputs, family_model_names, subjects_to_be_removed{condition_num}, total_subject_nums);
    ...
    savetofile(BMS_first_level_analysis,[main_dir filesep 'BMS_first_level_analysis_' condition_names{condition_num} '.mat']);
    save(fullfile(main_dir, inference_folder_name,'GCM.mat'),'GCM');
